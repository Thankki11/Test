...
exports.updateMenu = async (req, res) => {
  ...
  try {
    ...
    if (imageBuffer && fileName && mimeType) {
      ...
      const newImageUrl = saveImageToUploads(
        imageBuffer,
        fileName,
        mimeType,
        category
...
exports.createMenu = async (req, res) => {
  try {
    ...
    const imageUrl = saveImageToUploads(
      imageBuffer,
      fileName,
      mimeType,
      category
...
exports.addMenuReview = async (req, res) => {
  try {
    ...
    if (!orders || orders.length === 0) {
      return res
        .status(403)
        .json({ message: "Bạn phải mua sản phẩm này mới được đánh giá." });
    }
    // Thêm đánh giá
    const menu = await Menu.findById(id);
    if (!menu) return res.status(404).json({ message: "Menu not found" });
    ...
    res
      .status(201)
      .json({ message: "Đánh giá thành công!", reviews: menu.reviews });
  } catch (err) {
    res.status(500).json({ message: "Server Error" });
  }
};
